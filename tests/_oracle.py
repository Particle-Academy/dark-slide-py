"""The cross-runtime oracle: the PHP dark-slide writer, driven as a subprocess.
...
_PHP_SRC_ENV = "DARK_SLIDE_PHP_SRC"
...
def php_src_root() -> Path | None:
    ...
    sibling = REPO_ROOT.parent / "dark-slide" / "src"
...
def oracle_available() -> tuple[bool, str]:
    ...
    if php_src_root() is None:
        return False, f"the PHP dark-slide sources are not where {_PHP_SRC_ENV} or the sibling checkout says"
...
def php_to_bytes(payload: object) -> bytes:
    ...
    with tempfile.TemporaryDirectory(prefix="dark-slide-parity-") as tmp:
